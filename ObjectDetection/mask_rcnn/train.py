...
import transforms
from models.network_files import MaskRCNN
from models.backbone import resnet50_fpn_backbone
from datasets.my_dataset_coco import CocoDetection
...
def main(args):
    ...
    if len(train_loss) != 0 and len(learning_rate) != 0:
        from ObjectDetection.mask_rcnn.utils.plot_curve import plot_loss_and_lr
        plot_loss_and_lr(train_loss, learning_rate)

    # plot mAP curve
    if len(val_map) != 0:
        from ObjectDetection.mask_rcnn.utils.plot_curve import plot_map
        plot_map(val_map)
...
if __name__ == "__main__":
    ...
    # 训练数据集的根目录
    parser.add_argument('--data-path', default='../../DataBase/coco', help='dataset')
    # 检测目标类别数(不包含背景)
